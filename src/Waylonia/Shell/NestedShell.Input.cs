...
internal sealed partial class NestedShell
{
    ...
    public void HandleInput(in BasinViewInput input)
    {
        ...
        switch (input.Kind)
        {
            ...
            case BasinViewInputKind.TouchDown:
                MoveCursor(input.X, input.Y, input.TimeMs);
                OnButton(input.TimeMs, BtnLeft, true);
                break;
            case BasinViewInputKind.TouchMotion:
                MoveCursor(input.X, input.Y, input.TimeMs);
                break;
            case BasinViewInputKind.TouchUp:
                OnButton(input.TimeMs, BtnLeft, false);
                break;
    ...
    private void SetCursor(string name, bool overClient)
    {
        if (overClient)
        {
            ...
            {
                _overClient = true;
                ClientCursorChanged?.Invoke(null);
            }
    ...
    {
        "Super" => (_held & HotkeyModifiers.Super) != 0,
        "Ctrl" => (_held & HotkeyModifiers.Ctrl) != 0,
        _ => (_held & HotkeyModifiers.Alt) != 0,
    };
    ...
    private void HandleKey(uint time, uint code, bool pressed)
    {
        ...
        if (_switcherOpen)
        {
            if (!pressed && CaptureChord.ModifierOf(code) != HotkeyModifiers.None && (_held & HotkeyModifiers.Alt) == 0)
            {
                EndSwitcher(commit: true);
                return;
            }
    ...
    {
        var modifier = CaptureChord.ModifierOf(code);
    ...
    private void OnShapeRequested(CursorShape shape)
    {
        if (_overClient)
        {
            ClientCursorChanged?.Invoke(AvaloniaCursor.For(shape));
        }
    }
    ...
    private void OnCursorRequested(Basin.Seat.CursorRequest request)
    {
        ...
        if (request.Surface is not { } cursorSurface)
        {
            if (_overClient)
            {
                ClientCursorChanged?.Invoke(new global::Avalonia.Input.Cursor(global::Avalonia.Input.StandardCursorType.None));
            }
    ...
    {
        if (_overClient && AvaloniaCursor.FromSurface(surface, hotspotX, hotspotY) is { } cursor)
        {
            ClientCursorChanged?.Invoke(cursor);
        }
    }
}
