...
internal sealed partial class NestedShell : IDisposable
{
    private readonly BasinCompositorHost _host;
    private readonly Func<WlClient, string?> _sessionOf;
    ...
    public NestedShell(
        ...
        PanelLayout panel,
        KeyTable keys,
        Func<WlClient, string?> sessionOf,
        bool sessionTitles,
        ...
        BasinLogger log)
    {
        ...
        _frames = MetacityFrames.Load(settings);
        ...
        Output = new Box(0, 0, Logical(mode.Width, Scale), Logical(mode.Height, Scale));
        ...
        host.Screens.Advertise(view.Output, ScreenInfo(mode.Width, mode.Height, Scale));
    ...
    public bool SessionTitles { get; private set; }
    ...
    public event Action<global::Avalonia.Input.Cursor?>? ClientCursorChanged;
    ...
    public Action<string?, string, string?, Action<string?>>? IconResolver { get; set; }
    ...
    {
        if (IconResolver is not { } resolver)
        {
        ...
        resolver(window.Session, window.Content.AppId, window.ClientIcon, path => _post(() =>
        {
    ...
    private static HostScreenInfo ScreenInfo(int width, int height, double scale) =>
        new(OutputKey, OutputKey, 0, 0, width, height, scale, true);
    ...
    {
        var top = panel.Top.Count > 0 ? panel.Size : 0;
        var bottom = panel.Bottom.Count > 0 ? panel.Size : 0;
        return new Box(output.X, output.Y + top, output.Width, Math.Max(1, output.Height - top - bottom));
    }

    public Box TopStrip => new(Output.X, Output.Y, Output.Width, _panel.Top.Count > 0 ? _panel.Size : 0);

    public Box BottomStrip => new(
        Output.X, Output.Bottom - (_panel.Bottom.Count > 0 ? _panel.Size : 0), Output.Width, _panel.Bottom.Count > 0 ? _panel.Size : 0);
    ...
    public void Resize(int physicalWidth, int physicalHeight, double scale)
    {
        ...
        var mode = View.Output.CurrentMode;
        if (mode.Width == physicalWidth && mode.Height == physicalHeight && !scaleChanged)
        {
            return;
        }

        View.Resize(physicalWidth, physicalHeight, scale);
        _host.Screens.Advertise(View.Output, ScreenInfo(physicalWidth, physicalHeight, scale));
        Scale = scale;
        Output = new Box(0, 0, Logical(physicalWidth, scale), Logical(physicalHeight, scale));
        WorkArea = WorkAreaFor(Output, _panel);
    ...
    public void Apply(ShellSettings settings, PanelLayout panel, KeyTable keys, bool sessionTitles)
    {
        ArgumentNullException.ThrowIfNull(settings);
        ArgumentNullException.ThrowIfNull(panel);
        ArgumentNullException.ThrowIfNull(keys);
        ...
        if (!_frames.Matches(settings))
        {
            try
            {
                var frames = MetacityFrames.Load(settings);
                _frames.Dispose();
                _frames = frames;
                foreach (var window in _windows)
                {
            ...
            catch (Exception error) when (error is Basin.Frames.Metacity.MetacityThemeException or IOException or FormatException)
            {
                _log.Error($"the frame theme was not changed: {error.Message}");
            }
        }
    ...
    private void OnNewToplevel(XdgToplevelWindow toplevel)
    {
        ...
        var window = new ManagedWindow(this, content, ++_nextId, _sessionOf(client));
    ...
    private void OnMapped(ManagedWindow window)
    {
        ...
        RequestIcon(window);
        _log.Debug($"mapped '{window.Title}' at {window.FrameBox.X},{window.FrameBox.Y} decorated={window.Decorated}");
    ...
    private void OnUnmapped(ManagedWindow window)
    {
        ...
        window.Unmap();
        _log.Debug($"unmapped '{window.Title}'");
        if (_focused == window)
        {
            _focused = null;
    ...
    private void Place(ManagedWindow window)
    {
        ...
        var request = new PlacementRequest(
            ...
            parentTitle,
            Output);
        var origin = window.Content is ChromeContent { Centered: true }
    ...
    public void SetMinimized(ManagedWindow window, bool minimized)
    {
        ...
        _log.Debug($"{(minimized ? "minimized" : "restored")} '{window.Title}'");
    ...
    internal void OnFrameAction(ManagedWindow window, FrameAction action)
    {
        switch (action.Kind)
        {
    ...
    internal void OnFrameFaulted(ManagedWindow window, Exception error) =>
        _log.Error($"the frame of '{window.Title}' failed: {error.Message}");
    ...
    private static RenderColor BackgroundColor(ShellSettings settings) =>
        ShellColor.Parse(settings.Background) ?? ShellColor.Parse(ShellSettings.DefaultBackground)!.Value;
}
