...
# tools this needs, so that host runs build-waylonia.ps1 instead.
#
# A macOS target defaults to osx-universal, which publishes osx-x64 and
# osx-arm64 and lipo-merges every Mach-O file in them into one folder, so the
# zip runs on both kinds of Mac. Naming a concrete --rid builds that slice
# alone.
#
