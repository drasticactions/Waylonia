...
public sealed class SessionSettingsTests : IDisposable
{
    ...
    private Config Load(string toml)
    {
        ...
        return Config.Load(WayloniaPaths.ConfigOnly(path), BasinLogger.None);
    }
    ...
    public void A_flag_beats_the_session_file_which_beats_the_config_which_beats_the_default()
    {
        ...
        var fromDefault = Resolve(new SessionProfile("x", "h"), new SessionOverrides(), Config.Load(WayloniaPaths.Xdg().WithConfig(null), BasinLogger.None));
    ...
    public void Desktop_NAME_matches_a_saved_desktop_session_first()
    {
        ...
        var (profile, error) = RunRules.DesktopProfileFor("plasma", null, catalog, Load(""), ref overrides);
    ...
    public void Desktop_NAME_then_matches_a_desktops_profile_whose_host_names_a_session()
    {
        ...
        var (profile, error) = RunRules.DesktopProfileFor("work", null, catalog, config, ref overrides);
    ...
    public void Desktop_NAME_finally_matches_a_recipe_and_pulls_the_ssh_from_the_session()
    {
        ...
        var (profile, error) = RunRules.DesktopProfileFor("sway", "lab", catalog, Load(""), ref overrides);
        ...
        var (local, localError) = RunRules.DesktopProfileFor("sway", null, catalog, Load(""), ref overrides);
        Assert.Null(localError);
        Assert.Equal(string.Empty, local!.Ssh);

        var (_, unknown) = RunRules.DesktopProfileFor("gnome", null, catalog, Load(""), ref overrides);
