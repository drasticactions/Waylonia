...
public sealed class ConfigTests : IDisposable
{
    ...
    private static Config Load(string path) => Config.Load(WayloniaPaths.ConfigOnly(path), BasinLogger.None);
    ...
    public void Ssh_timeout_defaults_to_thirty_seconds_and_refuses_nonsense()
    {
        ...
        Assert.Equal(30, Config.Load(WayloniaPaths.ConfigOnly(Write("[host]\nssh-timeout = \"soon\"")), BasinLog.For("test")).SshTimeout);
    ...
    public void The_sessions_directory_sits_beside_the_config_file()
    {
        ...
        Assert.Null(Config.Load(WayloniaPaths.Xdg().WithConfig(null), BasinLogger.None).SessionsDirectory);
    }
    ...
    public void A_legacy_host_profile_is_kept_for_migration_and_warned_about()
    {
        ...
        var config = Config.Load(WayloniaPaths.ConfigOnly(Write("""
            [hosts.dev]
            ssh = "user@devbox"
            command = "tmux new -A -s main"
            compress = "none"
    ...
    public void Skipping_the_file_ignores_one_that_is_there()
    {
        ...
        var config = Config.Load(WayloniaPaths.Xdg().WithConfig(null), BasinLogger.None);
    ...
    public void The_default_path_gets_a_placeholder_that_parses_back_to_the_defaults()
    {
        ...
        try
        {
            var config = Config.Load(WayloniaPaths.Xdg(), BasinLogger.None);
            var path = Path.Combine(_directory, "waylonia", "waylonia.toml");

            Assert.True(File.Exists(path));
            Assert.Null(config.Socket);

            var reloaded = Config.Load(WayloniaPaths.Xdg(), BasinLogger.None);
