...
public sealed class SshSessionTests
{
    ...
    [Fact]
    public async Task An_ad_hoc_command_ending_disconnects_the_session_and_ends_it_with_its_code()
    {
        var host = new StubSessionHost { Settings = new HostSettings(TrayApps: false) };
        var settings = StubSessionHost.For("dev", adHoc: true) with { Command = "foot" };
        var (session, link, _) = await ConnectedAsync(host, settings);
        var ended = new List<int>();
        session.Ended += (_, code) => ended.Add(code);
        Assert.Equal(2, link.Commands.Count);

        link.Command(1).Exit(0);
        await WaitUntilAsync(() => ended.Count > 0, "the session noticed the command end");

        Assert.Equal([0], ended);
        Assert.True(session.CommandFinished);
        Assert.Equal(SessionStatus.Disconnected, session.Status);
        Assert.True(link.Disposed);
    }

    [Fact]
    public async Task An_ad_hoc_command_ending_reports_a_failure_as_one()
    {
        var host = new StubSessionHost { Settings = new HostSettings(TrayApps: false) };
        var settings = StubSessionHost.For("dev", adHoc: true) with { Command = "foot" };
        var (session, link, _) = await ConnectedAsync(host, settings);
    ...
    [Fact]
    public async Task An_ad_hoc_command_ending_keeps_the_session_while_another_client_it_launched_runs()
    {
        var host = new StubSessionHost { Settings = new HostSettings(TrayApps: false) };
        var settings = StubSessionHost.For("dev", adHoc: true) with { Command = "foot" };
        var (session, link, _) = await ConnectedAsync(host, settings);
        Assert.True(await session.LaunchAsync("firefox", "tray"));
        var ended = new List<int>();
        session.Ended += (_, code) => ended.Add(code);
        ...
        Assert.Empty(ended);
        Assert.False(session.CommandFinished);
        Assert.Equal(SessionStatus.Connected, session.Status);
    }

    [Fact]
    public async Task A_saved_session_keeps_running_after_its_command_ends()
    {
        var host = new StubSessionHost { Settings = new HostSettings(TrayApps: false) };
        var settings = StubSessionHost.For("dev") with { Command = "foot" };
        var (session, link, _) = await ConnectedAsync(host, settings);
        var ended = new List<int>();
        session.Ended += (_, code) => ended.Add(code);
