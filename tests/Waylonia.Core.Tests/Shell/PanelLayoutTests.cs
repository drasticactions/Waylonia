...
public sealed class PanelLayoutTests
{
    ...
    {
        var applets = PanelLayout.ParseApplets(
    ...
    public void An_unknown_name_warns_with_the_panel_and_the_applets_and_is_skipped()
    {
        ...
        var applets = PanelLayout.ParseApplets(["menu-bar", "foo", "clock"], "top", BasinLog.For("test"));
    ...
    {
        using var capture = new LogCapture();
        var applets = PanelLayout.ParseApplets(["launcher:", "launcher: "], "bottom", BasinLog.For("test"));
    ...
    {
        var layout = PanelLayout.From(new PanelSettings(Top: [], Bottom: []), BasinLogger.None);
    ...
    {
        var layout = PanelLayout.From(new PanelSettings(), BasinLogger.None);
    ...
    {
        Assert.Equal(new PanelApplet(PanelAppletKind.Clock), PanelLayout.ParseApplet(" clock "));
        Assert.Null(PanelLayout.ParseApplet("Clock"));
        Assert.Null(PanelLayout.ParseApplet(""));
    }
}
