...
public sealed class NestedShellTests
{
    ...
    public void A_mapped_client_is_framed_inside_the_work_area_and_the_panels_take_their_strips()
    {
        ...
        using var harness = new CompositorHarness(nested: new CompositorHarness.NestedShellOptions());
    ...
    {
        using var harness = new CompositorHarness(nested: new CompositorHarness.NestedShellOptions(1334, 1000, 1.6666666666666667));
        var shell = harness.Shell!;
    ...
    public void A_client_decorated_window_that_drops_its_shadow_when_maximized_stays_pinned_to_the_work_area()
    {
        ...
        var buffer = harness.Client.CreateBuffer(width, height, CompositorHarness.Fill(width, height, 0xFF3366AA));
    ...
    private static void Commit(CompositorHarness harness, HarnessToplevel toplevel, int width, int height)
    {
        var buffer = harness.Client.CreateBuffer(width, height, CompositorHarness.Fill(width, height, 0xFF3366AA));
        toplevel.Surface.Attach(buffer.Proxy, 0, 0);
        toplevel.Surface.Damage(0, 0, width, height);
    ...
    private static void CommitConfigured(CompositorHarness harness, HarnessToplevel toplevel, ManagedWindow window)
    {
    ...
    private static void Key(NestedShell shell, uint code, bool pressed) =>
        shell.HandleInput(new global::Basin.Hosted.BasinViewInput(
            global::Basin.Hosted.BasinViewInputKind.Key, 0, 0, 0, code, pressed, 0, 0, 0));
