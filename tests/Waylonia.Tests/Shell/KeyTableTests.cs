...
{
    private static KeyTable Build(IReadOnlyList<ShellKey> configured, IReadOnlyList<Hotkey>? hostHotkeys = null) =>
        KeyTable.Build(configured, hostHotkeys ?? [], BasinLogger.None);

    private static KeyTable BuildCapturing(IReadOnlyList<ShellKey> configured, IReadOnlyList<Hotkey> hostHotkeys, out List<string> lines)
    {
        using var capture = new LogCapture();
        lines = capture.Lines;
        return KeyTable.Build(configured, hostHotkeys, BasinLog.For("test"));
    }
    ...
    {
        var held = Enum.Parse<HotkeyModifiers>(modifiers);
    ...
    {
        using var capture = new LogCapture();
        var table = KeyTable.Build([], [], BasinLog.For("test"));
        ...
        Assert.Null(table.Match(HotkeyModifiers.None, 62));
        Assert.Null(table.Match(HotkeyModifiers.Alt | HotkeyModifiers.Ctrl, 62));
        Assert.Empty(KeyTable.Empty.Bindings);
        Assert.Null(KeyTable.Empty.Match(HotkeyModifiers.Alt, 62));
    }
    ...
    public void A_configured_chord_replaces_the_default_and_turns_a_silent_key_on()
    {
        ...
        Assert.Equal(new ShellKeyBinding("close", "Super+q", HotkeyModifiers.Super, 16), Binding(table, "close"));
        Assert.Equal(new ShellKeyBinding("tile-to-side-w", "Super+Left", HotkeyModifiers.Super, 105), Binding(table, "tile-to-side-w"));
        Assert.Null(table.Match(HotkeyModifiers.Alt, 62));
        Assert.Equal("close", table.Match(HotkeyModifiers.Super, 16));
    }
    ...
    public void An_empty_chord_disables_the_key()
    {
        ...
        Assert.Null(table.Match(HotkeyModifiers.Alt, 62));
        Assert.Null(table.Match(HotkeyModifiers.Alt, 67));
    }
    ...
    public void A_host_hotkey_on_the_same_chord_wins_and_is_reported_once()
    {
        ...
        var table = BuildCapturing([], hotkeys, out var lines);

        Assert.Equal(
            ["Warn: shell key close uses Alt+F4, which [hotkeys] already binds host-globally, keeping the hotkey"],
            lines.Where(line => line.StartsWith("Warn", StringComparison.Ordinal)));
        Assert.Null(table.Match(HotkeyModifiers.Alt, 62));
        Assert.DoesNotContain(table.Bindings, binding => binding.Name == "close");
        Assert.Equal("minimize", table.Match(HotkeyModifiers.Alt, 67));
    }
    ...
    public void The_chord_grammar_is_the_hotkey_one_over_evdev_codes(string chord, string modifiers, uint code)
    {
        ...
        Assert.Equal(Enum.Parse<HotkeyModifiers>(modifiers), parsedModifiers);
